...
from einops import rearrange,repeat,pack,unpack # 这里的rearrange是显示对象进行重排
from einops.layers.torch import Rearrange # 这是一个函数一般用于网络层中
...
class MultiHeadSlotAttention(Module):
    ...
    def forward(self,inputs,num_slots:int | None =None):
        ...
        for _ in range(self.iters):
            ...
            slots = slots + self.mlp(self.norm_pre_fft(slots))
        return slots
